export default {
    ...
    mounted() {
        console.log('eventsFormEdit mounted');
        ...
        if (element) {
            ...
            if (this.eventId) {
                ...
                console.warn('No eventId in data attribute');
                this.errors.general = 'No event ID found. Please go back and try again.';
                this.loading = false;
            }
        } else {
            console.error('Element #event-form-edit not found!');
            this.loading = false;
        }
    },
    methods: {
            ...
            <div v-else>
                ...
                <div v-for="slot in [1, 2, 3]" :key="slot" class="right-box drag-and-drop-con" style="margin-bottom: 1.5rem;">
                    ...
                    >
                        ...
                        <div v-else-if="!$data['imagePreview' + slot] && $data['currentImage' + slot]" class="image-preview">
                            <img :src="$data['currentImage' + slot]" :alt="formData['events_image_alt_' + slot]">
                            <button type="button" @click="replaceImage(slot)" class="remove-image-btn">Replace</button>
                        </div>
                    ...
                    <input
                        v-if="$data['imagePreview' + slot] || $data['currentImage' + slot]"
